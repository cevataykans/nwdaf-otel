...
// AssertContextElementConstraints checks if the values respects the defined constraints
func AssertContextElementConstraints(obj ContextElement) error {
	...
	for _, el := range obj.AggrSubs {
		if err := AssertSpecificAnalyticsSubscriptionConstraints(el); err != nil {
			return err
		}
	}
	for _, el := range obj.HistData {
		if err := AssertHistoricalDataConstraints(el); err != nil {
			return err
		}
	}
	for _, el := range obj.AdrfDataTypes {
		if err := AssertAdrfDataTypeConstraints(el); err != nil {
			return err
		}
	}
	for _, el := range obj.ModelInfo {
