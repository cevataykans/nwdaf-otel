...
type NwdafMlModelProvSubsc struct {
	...
	SuppFeats string `json:"suppFeats,omitempty" validate:"regexp=^[A-Fa-f0-9]*$"`
...
func AssertNwdafMlModelProvSubscRequired(obj NwdafMlModelProvSubsc) error {
	...
	for _, el := range obj.FailEventReports {
		if err := AssertFailureEventInfoForMlModelRequired(el); err != nil {
			return err
		}
	}
	return nil
...
// AssertNwdafMlModelProvSubscConstraints checks if the values respects the defined constraints
func AssertNwdafMlModelProvSubscConstraints(obj NwdafMlModelProvSubsc) error {
	for _, el := range obj.MLEventSubscs {
		if err := AssertMlEventSubscriptionConstraints(el); err != nil {
			return err
		}
	}
	for _, el := range obj.MLEventNotifs {
	...
	for _, el := range obj.FailEventReports {
		if err := AssertFailureEventInfoForMlModelConstraints(el); err != nil {
			return err
		}
	}
	return nil
