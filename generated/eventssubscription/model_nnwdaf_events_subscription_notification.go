// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

/*
 * Nnwdaf_EventsSubscription
 *
 * Nnwdaf_EventsSubscription Service API.   © 2025, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 
 *
 * API version: 1.2.4
 */

package eventssubscription




type NnwdafEventsSubscriptionNotification struct {
}

// AssertNnwdafEventsSubscriptionNotificationRequired checks if the required fields are not zero-ed
func AssertNnwdafEventsSubscriptionNotificationRequired(obj NnwdafEventsSubscriptionNotification) error {
	return nil
}

// AssertNnwdafEventsSubscriptionNotificationConstraints checks if the values respects the defined constraints
func AssertNnwdafEventsSubscriptionNotificationConstraints(obj NnwdafEventsSubscriptionNotification) error {
	return nil
}
